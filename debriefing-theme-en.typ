...
// Keep opening and closing quotation marks explicit in the English guide.
#let en-quote(body) = [“#body”]
...
// Keep each example together as one visually distinct practice area.
...
// Wrapped lines align with the prompt text rather than with the numeral.
...
// Keep the explanation as the main reading path and place optional prompts
// in a narrow, consistently styled reference area to its right.
#let question-layout(body, followups) = block(
  ...
  #grid(
    ...
    block(
      ...
      #text(size: 8.7pt, weight: "bold", fill: dark-blue)[Possible follow-ups]
      #v(0.45em)
...
// Match Quarto's callout structure while giving icon and title a clear gap.
...
#set page(
  header: context {
    if counter(page).get().first() > 1 {
      align(right)[
        #text(size: 8pt, fill: black)[Guide to Debriefing Simulation Games]
        #v(1.5pt)
